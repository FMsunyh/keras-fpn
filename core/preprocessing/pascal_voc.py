...
class PascalVocGenerator(Generator):
    def __init__(
        ...
        **kwargs
        ...
        self.image_names          = [l.strip().split(None, 1)[0] for data_dir in self.data_dirs for l in open(os.path.join(data_dir, 'ImageSets', 'Main', set_name + '.txt')).readlines()]
        self.image_dirs           = [data_dir for data_dir in self.data_dirs for l in open(os.path.join(data_dir, 'ImageSets', 'Main', set_name + '.txt')).readlines()]
